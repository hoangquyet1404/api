...
exports.index = async (req, res, next) => {
    class Gemini {
        ...
        constructor(cookie, config = {}) {
            ...
            this.#fetch = config.fetch || require('node-fetch');
            this.cookie = cookie;
        ...
        async #uploadImage(name, buffer) {
            ...
            try {
                ...
                // Lưu imageFileLocation vào thư mục data
                const fs = require('fs').promises;
                const path = require('path');
                const filePath = path.join(this.dataDir, `${name}.txt`);
                await fs.writeFile(filePath, imageFileLocation);
