...
async function search(keyword) {
  ...
  try {
    ...
    console.log('Response data:', response.data); // In toàn bộ dữ liệu trả về để kiểm tra

    // Kiểm tra cấu trúc dữ liệu trả về
    if (!response.data || !response.data.data || !response.data.data.video_templates) {
      throw new Error('Dữ liệu trả về không có video_templates');
    }

    // Trả về kết quả đã xử lý
    const results = response.data.data.video_templates.map(template => ({
      id: template.web_id,
      ...
      comment_count: template.interaction ? template.interaction.comment_count : 0,
      create_time: template.create_time,
      author: {
        unique_id: template.author ? template.author.unique_id : 'N/A',
        name: template.author ? template.author.name : 'N/A',
      },
...
// Định nghĩa route handler
exports.name = '/capcut/search';
exports.index = async (req, res, next) => {
    ...
    try {
        ...
        res.json({ error: 'Có lỗi xảy ra khi tìm kiếm' });
    }
};
