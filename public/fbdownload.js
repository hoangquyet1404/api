...
async function readTokens() {
    try {
        ...
        const { EAAAAU, EAAD6V7 } = JSON.parse(tokenData);
        return { EAAAAU, EAAD6V7 };
    } catch (error) {
    ...
    stories(data, storyID) {
        return {
            ...
            attachments: data?.bucket?.unified_stories?.edges.map(item => ({
                ...
                url: item?.node?.attachments?.[0]?.media?.__typename === 'Photo' 
...
const Utils = {
    postWithToken: async (url, form, userAgent) => {
        return await axios.post(url, form, {
            headers: {
                'authorization': 'OAuth ' + global.account.token.EAAAAU,
